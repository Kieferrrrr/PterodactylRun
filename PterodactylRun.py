...
if OS() == "Windows":
    cl = "cls"
    os.system("mode 80,20 && title Pterodactly Run - Version 1.0.4 - CLI")
else:
    cl = "clear"
...
class pterodactyl(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect() # Getting the screen x, y, width, height
...
class main:
    def __init__(self):
        self.datastore = dataHandler()
        self.fps, self.savescores = self.datastore.getINI()
        self.sprites = pygame.sprite.Group()
        self.player = pterodactyl()
        self.sprites.add(self.player)
        self.clock = pygame.time.Clock()
        self.screenCTRL = pygame.display.set_mode((vw, vh))
        self.highscore, self.lastscore = self.datastore.getJSON()
        self.score = 00000
